...
def _check_tool_installed(command: str, display_name: str) -> bool:
    ...
    try:
        ...
        if available:
            # Log the version string (first line of stdout or stderr)
...
async def lifespan(app: FastAPI):
    ...
    logger.info("  Tesseract   : %s", "✓" if TESSERACT_AVAILABLE else "✗ (Image→Excel disabled)")
    logger.info("  LibreOffice : %s", "✓" if LIBREOFFICE_AVAILABLE else "✗ (Word→PDF disabled)")
    logger.info("  CORS origins: %s", ALLOWED_ORIGINS)
    logger.info("=" * 60)

    # Launch background task that periodically cleans expired temp files
...
async def _periodic_cleanup():
    """
    Background coroutine that runs every CLEANUP_INTERVAL_SECONDS
    and removes temp files older than the configured TTL.
    """
    while True:
...
async def health_check():
    """
    Health check endpoint — used by deploy platforms and monitoring.
    Reports whether required external tools (Tesseract, LibreOffice) are available.
    """
...
async def download_file(filename: str):
    ...
    Path traversal attempts are rejected. The original (user-friendly)
    filename is sent via the Content-Disposition header.

    After the first download, the file is scheduled for deletion after
    a 30-second grace period (to handle slow connections or retries).
    """
    # ── Security: reject anything that could be a path traversal ──
    if (
        "/" in filename
        or "\\" in filename
        or ".." in filename
    ...
    if not entry or not entry.file_path.exists():
        raise HTTPException(
            status_code=404,
            detail="File not found or has expired. Conversions are kept for 10 minutes.",
        )
    ...
    # After the first download, schedule deletion with a grace period
    # This handles: slow connections, browser retry on interruption, etc.
    ...
    return FileResponse(
        ...
        # Content-Disposition: attachment is set automatically by FileResponse
...
async def _delete_after_download(
    uuid_filename: str,
    file_path: "import pathlib; pathlib.Path",
    delay_seconds: int,
):
    """
    Wait for the specified delay, then delete the file and remove it
    from the registry. This ensures the download can complete (including
    potential retries) before the file disappears.
    """
    ...
    try:
        ...
        logger.warning("Post-download cleanup failed for '%s': %s", uuid_filename, e)


# ═══════════════════════════════════════════════════════════════════════════════
# Error Handlers — ensure all errors return consistent JSON
# ═══════════════════════════════════════════════════════════════════════════════

@app.exception_handler(HTTPException)
async def http_exception_handler(request, exc: HTTPException):
    """Convert FastAPI HTTPExceptions to our standard ErrorResponse format."""
    return JSONResponse(
...
# ═══════════════════════════════════════════════════════════════════════════════
# Routers — conversion endpoints (added in Phase 3)
# ═══════════════════════════════════════════════════════════════════════════════

# These will be uncommented in Phase 3 when the route files are created:
#
